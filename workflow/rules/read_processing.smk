rule indexing:
    message:
        "Indexing the human reference genome: {wildcards.id}"
    conda:
...
rule fastp:
    ...
    output:
        ...
        report="results/{id}/{id}.fastp.json"
    threads: 4
    shell:
        "fastp --in1 {input.r1} --in2 {input.r2} --out1 {output.r1_trimmed} --out2 {output.r2_trimmed} --report_title {wildcards.id} --thread {threads} --json {output.report} > {log} 2>&1" 
...
rule map_to_human_genome:
    ...
    input:
        r1=rules.fastp.output.r1_trimmed,
        r2=rules.fastp.output.r2_trimmed,
